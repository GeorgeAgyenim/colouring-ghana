...
|---|---|---|---|---|
| Playwright | to be pinned (dev dependency) | Scripted map benchmark path (`docs/benchmarks/map-migration-method.md`) and, later, parity checks | Apache-2.0 | Planned (E0). Alternatives: Lighthouse (no tile-latency on a pan path, no phone); manual timing (not reproducible). |
